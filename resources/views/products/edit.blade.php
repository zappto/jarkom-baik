...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        {{-- Form --}}
        <form method="POST" action="{{ route('products.update', $product) }}" class="px-6 py-6 space-y-6">
            @csrf
            @method('PATCH')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" id="purchase_price" name="purchase_price" value="{{ old('purchase_price', $product->purchase_price) }}" min="0" step="0.01"
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-400 text-sm">$</span>
                        <input type="number" id="selling_price" name="selling_price" value="{{ old('selling_price', $product->selling_price) }}" min="0" step="0.01"
                ...
                <div>
                    <label for="current_stock_display" class="block text-sm font-medium text-gray-700 mb-1">Current Stock</label>
                    <div class="relative">
                        <input type="text" id="current_stock_display"
                               value="{{ $product->current_stock }}" readonly disabled
                               class="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm text-gray-500 cursor-not-allowed">
                        <input type="hidden" name="current_stock" value="{{ $product->current_stock }}">
                    </div>
                    <p class="mt-1 text-xs text-gray-400">Stock quantity cannot be edited here. Use incoming/outgoing stock to adjust.</p>
                </div>
