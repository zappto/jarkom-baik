    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <div class="flex items-center gap-3 flex-1">
                <form method="GET" action="{{ route('incoming-stock.index') }}" class="flex items-center gap-3">
            ...
            <a href="{{ route('incoming-stock.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors duration-150">
        ...
        <div class="p-5">
            @if ($incomingStocks->isEmpty())
                <p class="text-sm text-gray-500 text-center py-8">No incoming stock records found.</p>
            @else
                <table class="w-full">
                    <thead>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach ($incomingStocks as $stock)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="py-3 pr-4">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">+{{ $stock->quantity }}</span>
                                </td>
                                <td class="py-3 pr-4">
                                    <span class="text-sm text-gray-600">{{ $stock->supplier?->name ?: '-' }}</span>
                                </td>
                                <td class="py-3 pr-4">
                                    <span class="text-sm text-gray-500">{{ $stock->notes ?: '-' }}</span>
                                </td>
                                <td class="py-3 text-right">
                                    <form action="{{ route('incoming-stock.destroy', $stock) }}" method="POST" onsubmit="return confirm('Delete this record? This will decrease product stock.')">
                                        @csrf
                                        @method('DELETE')
