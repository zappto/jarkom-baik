    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <div class="flex items-center gap-3 flex-1">
                <form method="GET" action="{{ route('suppliers.index') }}" class="flex-1 max-w-xs">
            ...
            <a href="{{ route('suppliers.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors duration-150">
        ...
        <div class="p-5">
            @if ($suppliers->isEmpty())
                <p class="text-sm text-gray-500 text-center py-8">No suppliers found.</p>
            @else
                <table class="w-full">
                    <thead>
                        <tr class="text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            <th class="pb-3 pr-4">Name</th>
                            <th class="pb-3 pr-4">Contact Person</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach ($suppliers as $supplier)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="py-3 pr-4">
                                    <span class="text-sm text-gray-600">{{ $supplier->products_count }}</span>
                                </td>
                                <td class="py-3 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('suppliers.edit', $supplier) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors duration-150">
